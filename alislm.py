...
def make_model(c):
    # [FUNCTIONAL] was alisLM(nn.Module).__init__; now a factory returning a plain dict.
...
def model_forward(model, ids, past=None, targets=None):
    # [FUNCTIONAL] was alisLM.forward(self, ids, past, targets); now model_forward(model, ids, ...)
...
def model_generate(model, ids, max_new=80, temperature=0.8, top_k=40):
    # [FUNCTIONAL] was alisLM.generate(self, ids, ...); now model_generate(model, ids, ...)
